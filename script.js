...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ========== ASSESSMENT FORM — WEB3FORMS SUBMISSION ========== */
  const intakeForm = document.getElementById('intake-form');
  const formStatus = document.getElementById('form-status');

  if (intakeForm) {
    const submitBtn = intakeForm.querySelector('.form-submit');
    const submitText = intakeForm.querySelector('.form-submit-text');
    const originalText = submitText ? submitText.textContent : 'Submit Intake';
    ...
    intakeForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (response.ok && result.success) {
          setStatus('Intake received. Expect a reply within two business days.', 'success');
          intakeForm.reset();
          // Smooth scroll status into view
          formStatus.scrollIntoView({ behavior: 'smooth', block: 'center' });
        } else {
          const msg = (result && result.message) ? result.message : 'Something went wrong. Please try again or email hello@businessoutcomeengineering.com.';
          setStatus(msg, 'error');
        }
      } catch (err) {
        setStatus('Network error. Please try again or email hello@businessoutcomeengineering.com.', 'error');
      } finally {
